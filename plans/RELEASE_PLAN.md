...
- ✅ **C4 · AI definition/sentence enrichment adapter + run** — DONE (`ed6791c` code; `0cc4d45` run). DeepSeek-chat enriched 2,790 TBD stubs (3 passes) + 10 manual strict-validation fixes. 2,881 words / 0 TBD stubs. `validate --strict` clean. `words.db` copied to `mobile/assets/vocab/`. *Files:* `providers/anthropicDefinitionProvider.ts`, `commands/enrich.ts`.
- ✅ **C5 · Human review at realistic scale** — DONE. `validate --strict` clean (0 errors, 2,802 known warnings: 2,798× `theme='General'` placeholder + 4× AI `_s` glue sentences — triaged, do not re-investigate). Words.db in mobile bundle. *Deps:* C4.
...
- ✅ **C8 · Repeatable content-release process** — DONE. New `release` command + npm script: `import → enrich → validate --strict → export(+version bump) → copy to mobile/assets/vocab/words.db`, fail-closed (strict validate runs FIRST, throws before export; copy is last + only on clean export — unit-tested: failing validate → copy never runs). Flags `--bump major|minor|patch`, `--no-copy`, `--provider`. Enrich stays offline-Noop unless `--provider openai` + key. The reusable path for P6 drops. *Deps:* C0, C1, C7.
...
- ◑ **B3 · Source maps + release health + alerts** — WIRED + SECRET SET; only build-verify left. Sentry org `rg007` / project `lexitap` (US host) created. DONE: `@sentry/react-native/expo` plugin added to `app.config.ts` **conditional on `SENTRY_ORG`+`SENTRY_PROJECT`** (verified: plugin present with creds, absent without → Expo Go/credential-less builds don't break); `SENTRY_ORG`/`SENTRY_PROJECT` + safe-to-ship `EXPO_PUBLIC_SENTRY_DSN` added to `eas.json` preview+production `env`; DSN + org/project + `SENTRY_AUTH_TOKEN` in gitignored `.env.local` for local source-map upload. `authToken` read from the `SENTRY_AUTH_TOKEN` env at build time (never hardcoded). **`SENTRY_AUTH_TOKEN` now set as an EAS project secret env var** (`@rmg007/lexitap`, secret-type, builder-only; verified via `eas env:list production`). 272 mobile tests green. **REMAINING:** (1) run a `preview`/`production` EAS build → confirm source maps upload to Sentry + a test crash symbolicates (this is also the C0 fresh-build); (2) optional: release-health alerts in Sentry UI; (3) **rotate the auth token** — it transited a chat transcript. *Deps:* B1 ✅, eas.json ✅, EAS secret ✅.
