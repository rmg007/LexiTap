...
- ◑ **B3 · Source maps + release health + alerts** — WIRED, awaiting EAS secret + device-verify. Sentry org `rg007` / project `lexitap` (US host) created. DONE: `@sentry/react-native/expo` plugin added to `app.config.ts` **conditional on `SENTRY_ORG`+`SENTRY_PROJECT`** (verified: plugin present with creds, absent without → Expo Go/credential-less builds don't break); `SENTRY_ORG`/`SENTRY_PROJECT` + safe-to-ship `EXPO_PUBLIC_SENTRY_DSN` added to `eas.json` preview+production `env`; DSN + org/project + `SENTRY_AUTH_TOKEN` in gitignored `.env.local` for local source-map upload. `authToken` is read from the `SENTRY_AUTH_TOKEN` env at build time (never hardcoded). 272 mobile tests green. **REMAINING (needs Ryan/EAS auth):** (1) `eas secret:create --scope project --name SENTRY_AUTH_TOKEN --value <token> --type string` (token is gitignored-only here; EAS CLI not authed in this env); (2) run a `preview`/`production` EAS build → confirm source maps upload to Sentry + a test crash symbolicates; (3) optional: release-health alerts in Sentry UI. *Deps:* B1 ✅, eas.json ✅.
