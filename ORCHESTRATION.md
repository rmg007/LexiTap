...
> **2026-07-10 ✅ — CONTENT-3 IELTS done** (plan [`IELTS_INGEST_PLAN.md`](plans/IELTS_INGEST_PLAN.md), [memory note](memory/2026-07-10_ielts-ingest.md)). 683 existing words tagged `ielts`/`ielts-extended` + CEFR-backfilled; 2,317 net-new words ingested, enriched via `gpt-4o-mini` (7 passes), adversarially reviewed (8-agent parallel + verify, 36 confirmed findings fixed). Shipped `words.db` user_version=6, 6,566 words (was 4,332). content-tool 308 / mobile 617 tests green. Commit `fcc9eb1`. **Remaining CONTENT-3 stubs: gre/gmat/business/advanced/common9k — same Ryan-sourcing blocker (5 tiers).**

> **2026-07-10 ✅ — Foundation CEFR backfill done** (plan [`FOUNDATION_ELIMINATION_PLAN.md`](plans/FOUNDATION_ELIMINATION_PLAN.md), [memory note](memory/2026-07-10_foundation-cefr-backfill.md)). 2,038 foundation-only words with no CEFR received levels via `categorize --tier foundation --model gpt-4.1-mini`; 7 adversarial corrections + 2 model-missed words hand-assigned; 0 foundation words now lack CEFR (100% coverage, was 2%). words.db bumped to user_version=8 (same 6,566 words; pure categorization, no count change). content-tool 308 / mobile 617 tests green. Commit `61dd199`. **Follow-up (open): revisit whether CEFR-band ordering should replace `difficulty ASC` in `selectNewWords` now that coverage is 100%.**
...
### CONTENT-3 · Specialty-tier cross-reference (TOEFL/IELTS/…) — **TOEFL + IELTS done, 5 tiers remain**
```
...
```
**`toefl` done 2026-07-06; `ielts` done 2026-07-10.** Both used the same playbook: offline filter (dedup/lemmatize/zipf-obscurity) → stub ingest → `enrich-master` (base fields + senses + quiz questions) → adversarial review → `validate --strict` + `release`. IELTS result: 683 existing words tagged + 2,317 net-new words shipped (`ielts`/`ielts-extended` tiers); 36 adversarial findings fixed; words.db v6, 6,566 words. See [memory note](memory/2026-07-10_ielts-ingest.md) and [plan](plans/IELTS_INGEST_PLAN.md).
**Remaining: gre/gmat/business/advanced/common9k are still ~5-word stubs (5 tiers).** The blocker per tier is **Ryan sourcing (or approving) a real word list** — the pipeline is proven and agent-runnable once a list exists. `owner: ryan` reflects that sourcing step, not any remaining code work.
...
*Maintained by `/orchestrate`. Last sync: 2026-07-11 (IELTS ingest done — CONTENT-3 now TOEFL+IELTS done, 5 tiers remain; Foundation CEFR backfill done — 100% CEFR coverage on foundation, words.db v8 6,566 words). No litter. Source of task-level truth for dependencies remains [`plans/RELEASE_PLAN.md`](plans/RELEASE_PLAN.md); this file is the runnable projection of it.*
