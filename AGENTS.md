...
- **`import-master` is a full upsert; children are clean-slate.** Per word: word fields + `cefr_level` + `reviewed` + synonyms/antonyms are overwritten, and `word_tiers` / `word_senses` / `sense_examples` / `word_questions` are deleted-then-reinserted. The master file is authoritative — removing a tier/sense/question from a line removes it from the DB on re-import. Word IDs stay `makeWordId(word)` (surface-form only), so review history survives a rebuild.
