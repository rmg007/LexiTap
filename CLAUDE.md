...
`git clone` + `npm install` + `git config pull.rebase true` (push-race protocol — `session-context.sh` nags if unset) is all you need. All Claude Code config is in the repo:
...
| Destructive git: force-push (incl. `+refspec` form), `git branch -D`, `git worktree remove --force`, `git stash clear`/`drop`, deleting the `main` ref | One bad command from any parallel session destroys shared history or another session's uncommitted work. The server-side `protect-main` ruleset backs this up on `main`; the hook also covers non-main refs. (Added 2026-06-11.) |

The hook has a CI-enforced self-test — `node .claude/hooks/guardrails-selftest.mjs` (runs in `.github/workflows/infra.yml`) — because the hook **fails open**: without the self-test, a syntax error would silently disable every rule above.
...
|---|---|
| Merged branches + worktrees pile up and look like lost work | **`/orchestrate sync` step 8 reaps merged litter** (worktrees + local + remote branches). GitHub's auto-delete-on-merge stays enabled but **never fires for our CLI merges** (no PRs) — the reap step is the real defense. `session-context.sh` reports litter, stranded branches, and stashes at every session start. |
| A PR is closed without merging, discarding real work | Weekly audit or manual check before closing any PR |
| Commits made locally but never pushed | The **`Stop` hook** warns when `HEAD` is ahead of upstream (fixed 2026-06-11 — it previously tested the dirty tree, not unpushed commits) and when the branch has no upstream at all; the **`SessionStart` hook** (`session-context.sh`) re-warns at the next session start; the **status line** shows `↑N` unpushed at all times |
| A force-push or branch deletion rewrites/destroys public `main` history | **Server-side GitHub ruleset `protect-main`** (blocks force-push + deletion on `main`; applies to admins; emergency escape hatch = temporarily disable it in repo Settings → Rules, e.g. to purge a committed secret) + `guardrails.mjs` blocks force-push (incl. `+refspec`), `branch -D`, `worktree remove --force`, `stash clear`/`drop` at the tool boundary |
| A secret lands in any committed file (not just `.env`) | **GitHub secret scanning + push protection** (repo Settings → Code security; these are repo-level settings that do NOT travel with the repo — never disable) + the `git add` .env guardrail |
| Work left in a `git stash` dies silently with its worktree | **Never leave work in a stash across sessions** — stashes are per-worktree, invisible to all other defenses, and become dangling commits when a worktree is pruned. Commit to a branch instead. `session-context.sh` reports stash count. |
...
- **Push-race protocol (parallel sessions pushing to `main`):** set `git config pull.rebase true` once per clone (New Machine Setup; `session-context.sh` nags if unset) so a lost push race rebases your commit instead of minting a duplicate-message merge commit. Exception: right after an **octopus batch merge**, a lost race must use `git pull --no-rebase` (rebase would flatten the merge), then push. Merge-commit subjects use one convention: `chore(merge): <branch> → main`.
