...
- **"Review your work" / "review your code" / "check your work" / "review and fix" / "fix this bug" / "fix bugs" / "fix this error" / "fix errors" / pasted stack trace → invoke `/review-and-fix` immediately.** Always: (1) find + verify + fix every CONFIRMED/PLAUSIBLE finding, (2) prevent recurrence — add rules to CLAUDE.md/AGENTS.md, block in guardrails.mjs if warranted, add tests, write a memory note. Never return a findings list and stop. The command lives at [`.claude/commands/review-and-fix.md`](.claude/commands/review-and-fix.md).
