...
describe('StreakEventUseCase', () => {
  ...
  it('emits streak_incremented when streak increases', async () => {
    ...
    expect(mockAnalytics.track).toHaveBeenCalledWith('streak_incremented', {
      current_streak: 5,
      longest_streak: 10,
    });
  });

  it('emits streak_broken when streak resets', async () => {
    await useCase.execute({
      event: 'broken',
      newStreak: 0,
      longestStreak: 10,
    ...
    expect(mockAnalytics.track).toHaveBeenCalledWith('streak_broken', {
      current_streak: 0,
      longest_streak: 10,
    });
  });
