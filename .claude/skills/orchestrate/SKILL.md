...
1. **Verify the task actually passed its `verify` condition.** Run the stated check (e.g. `npm run check` in the affected project). If it didn't pass, the task is NOT done — leave it `in-progress`, report what failed, stop. Do not mark green what isn't.
