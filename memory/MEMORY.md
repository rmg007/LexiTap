...
- **B — contrast FLAGGED (not auto-applied — token-value change = Ryan's call).** Real AA failures after filtering noise (`on-accent` sits on accent fill; `*/subtle` are bg tints; `border/strong` is a 3:1 border): **`text/tertiary` #6e777b fails AA in DARK (canonical) mode — 3.43–4.28:1 vs 4.5** on every surface; **`accent` #178f88 as text in LIGHT = 3.8–3.95:1** (passes large 3:1 only). Fix lives in `tokens.ts` (canonical — Figma ports, never invents) → ripples every screen + accent touches brand → presented to Ryan, not silently changed.
