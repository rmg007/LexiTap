...
- ⚠️ **Left intact (surfaced, not deleted):** the Hi-Fi page still has many **pre-existing duplicate whole-screen Hi-Fi frames** (e.g. Welcome `83:2`+`113:2`, Age Gate, Goal, O-9, SE-/SP-/W- families appear twice). These are NOT from my session and may be an intentional grid/index — did not mass-delete. Ryan's call if he wants them pruned.

**Working-tree cleanup — DONE (2026-06-09):** Reverted throwaway root `package.json`+`package-lock.json` (`sharp`, one-time image tooling; violated CLAUDE.md "No root package.json"; npm-regenerable). Gitignored brand-asset *exploration* scratch (`iconikai-icon-pack.zip`, `lexitap_svg_assets/`, `.DS_Store`) — kept on disk, out of git; canonical icons already tracked in `website/assets/lexitap-icon-*`. Left untracked: `website/assets/lexitap-app-store-ios.png` + `lexitap-google-play.png` (store badges, NOT yet referenced by any website HTML → belong to the website work stream, not committed here).
